...
export const MobileDock = ({ items }: IMobileDockProps) => {
  return (
    <div className="block lg:[@media(hover:hover)]:hidden fixed bottom-0 left-0 right-0 z-10">
      <div className="w-full bg-glass-blur dark:bg-glass-blur-dark backdrop-blur-glass px-4 pb-7 pt-3 theme-transition">
        <div className="grid grid-cols-4 gap-2 max-w-sm mx-auto">
          {items.map((item) => (
            <button
              key={item.label}
              onClick={item.onClick}
              className="group relative w-full @container px-3 py-2 rounded-xl cursor-pointer bg-button-light dark:bg-button-dark active:bg-button-light/80 active:dark:bg-button-dark/80 touch-none active:scale-95 transition-all duration-200 [-webkit-tap-highlight-color:transparent] theme-transition"
            >
